...
if len(sys.argv) >= 2 and sys.argv[1] != "partial" and sys.argv[1] != "full":
    print("Unknown options; stopping execution")
    exit(1)
...
def save_sklearn_kdd(percent10=False, subset=None, filename="output.csv"):
    ...
    dataset = fetch_kddcup99(
        percent10=percent10, subset=subset, data_home=data_dir, download_if_missing=True
    )
...
# 10% Training Dataset
if len(sys.argv) < 2 or sys.argv[1] == "partial":
    save_sklearn_kdd(percent10=True, filename="kddcup_10_percent.csv")

# Full Training Dataset (~4.9 million rows)
if len(sys.argv) < 2 or sys.argv[1] == "full":
    save_sklearn_kdd(percent10=False, filename="kddcup_full.csv")

# Corrected Test Dataset - uncomment to run
# save_sklearn_kdd(
#    percent10=False,
#    subset="corrected",
